...
for ( i = 0; i < jobSearch.hits.length; i ++){
 console.log(`#${i+1}.`)
 console.log(`Job title: ${jobSearch.hits[i].jobTitle}`)
 console.log(`Location: ${jobSearch.hits[i].locations}`)
 console.log(`Salary: ${jobSearch.hits[i].jobSalary}`)
 console.log('Benefits:')
 for (j in jobSearch.hits[i].benefits ){
       console.log(` ${jobSearch.hits[i].benefits[j].benefitValue}`)
 }
 console.log(`Skills: ${jobSearch.hits[i].skills}`)
 console.log(`Requirements: ${jobSearch.hits[i].jobRequirement}`)
}


//Ex 4.1 + 4.2:
alert('Hi there, this is dev dictionary')
let dic = {dev: "The process of figuring out why your program has a certain error and how to fix it"}
...
if (input == 'debug'){
    ...
    new_word = prompt(`We could not find your word: ${input}, leave your explanation`)
    if (new_word === ""){
     alert(`You can add definition to ${input}`)
    } else {
       alert('Done')
       let new_input = prompt('Enter a keyword').toLowerCase()
       if (input == `${new_input}`){
         alert(`${input}\n ${new_word}`)}
    }
}
...
for ( i in dic){
    if (i == input){
        ...
        let new_word = prompt(`We could not find your word: ${input}, leave your explanation`)
        alert('Done')
        dic[input] = new_word
        break
    }
} 
let input2 = prompt('Enter a keyword').toLowerCase()
for ( i in dic){
    if (i == input2){
        alert(dic[i])}}
...
for ( j=0; j <products.length; j++ )
 if (position == j+1){
    let {name, Brand, Price, Color, Category} = products[j]
    console.log(`Name: ${name}\nBrand: ${Brand}\nPrice: ${Price}\nColor: ${Color}\nCategory: ${Category}`)
}
...
//Ex 5.4:
products[0].Providers = ['Phukienzeo', 'Dientuccc']
...
//Ex 5.5:
let cate = prompt('Enter your category').toLowerCase()
let provi = prompt("Enter provider")
for (i = 0; i< products.length; i++){
    if (products[i].Category.toLowerCase() == cate){              
        if (products[i].Providers.indexOf(provi) != -1){
            console.log('-------------------------')
